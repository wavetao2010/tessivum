...
const TAG: &str = "v0.1.0-alpha.20";
const TARGET: &str = "x86_64-unknown-linux-gnu";
const MARKET_VERSION: &str = "0.1.0-alpha.20";
const MARKET_FILENAME: &str = "tessivum-market-0.1.0-alpha.20.tgz";
...
impl Fixture {
    ...
    fn archive(&self) -> PathBuf {
        self.output
            .join("tessivum-0.1.0-alpha.20-x86_64-unknown-linux-gnu.tar.gz")
    }

    fn stage(&self) -> PathBuf {
        self.output
            .join("tessivum-0.1.0-alpha.20-x86_64-unknown-linux-gnu")
    }
}
...
fn release_archive_contains_compatibility_assets_without_legacy_preset_assets() {
    ...
    for path in [
        ...
        "bin/tsv",
        "libexec/tessivum",
        "share/tessivum/plugins/tessivum-market-0.1.0-alpha.20.tgz",
        "share/tessivum/plugins/tessivum-market-0.1.0-alpha.20.tgz.sha256",
        "share/tessivum/plugins/tessivum-market-0.1.0-alpha.20.tgz.source.json",
        "share/licenses/tessivum-market-0.1.0-alpha.20/LICENSE",
    ...
    let market_tgz = fixture
        .stage()
        .join("share/tessivum/plugins/tessivum-market-0.1.0-alpha.20.tgz");
    ...
    assert_eq!(
        fs::read(
            fixture
                .stage()
                .join("share/tessivum/plugins/tessivum-market-0.1.0-alpha.20.tgz.source.json",)
        )
        .unwrap(),
    ...
    assert_eq!(
        fs::read(
            fixture
                .stage()
                .join("share/licenses/tessivum-market-0.1.0-alpha.20/LICENSE"),
        )
        .unwrap(),
        fs::read(repository_root().join("packaging/licenses/dsh-market/LICENSE")).unwrap(),
    );
    ...
    assert!(
        launcher_text.contains("$root/share/tessivum/plugins/tessivum-market-0.1.0-alpha.20.tgz")
    );
    assert!(launcher_text.contains("TESSIVUM_MARKET_SHA256_FILE"));
    assert!(launcher_text.contains("tessivum-market-0.1.0-alpha.20.tgz.sha256"));
    assert!(launcher_text.contains("TESSIVUM_MARKET_SOURCE_FILE"));
    assert!(launcher_text.contains("tessivum-market-0.1.0-alpha.20.tgz.source.json"));
