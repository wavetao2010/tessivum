...
fn write_matrix_private_temp_isolated_and_stale_cleanup_is_safe() {
    ...
    let mut live = spawn(
        ...
        &first,
        format!(
            "$note = {}; [IO.File]::WriteAllText(($note + '.tmp'), $env:TEMP, [Text.UTF8Encoding]::new($false)); Move-Item -LiteralPath ($note + '.tmp') -Destination $note; Set-Content -LiteralPath (Join-Path $env:TEMP 'own.txt') -Value ok; Set-Content -LiteralPath {} -Value ok; Start-Sleep -Seconds 60",
            ps(&temp_note),
...
fn runner_job_kills_started_descendant_after_parent_exits() {
    ...
    let descendant = format!(
        "$ErrorActionPreference='Stop'; $held = [IO.File]::Open((Join-Path $env:TEMP 'held.txt'), [IO.FileMode]::Create, [IO.FileAccess]::ReadWrite, [IO.FileShare]::None); [IO.File]::WriteAllText({}, $env:TEMP, [Text.UTF8Encoding]::new($false)); $note = {}; Set-Content -LiteralPath ($note + '.tmp') -Value $PID -NoNewline; Move-Item -LiteralPath ($note + '.tmp') -Destination $note; Start-Sleep -Seconds 300",
        ps(&temp_file),
        ps(&pid_file),
    );
    let script = format!(
