...
test('runs the Minimal Agent Mode with its restricted tools through the native agent', async () => {
  ...
  try {
    ...
    const history = await waitUntil(
      () => harness.rpc<{ events: HistoryEvent[] }>('session.history', { sessionId, maxMessages: 1_000 }),
    ...
    expect(JSON.stringify(events.filter(({ event }) => event.type === 'assistant/message' || event.type === 'assistant/chunk'))).toContain('MINIMAL_MODE_REQUEST_OK')
    harness.assertClean()
  } finally {
    await harness.close()
  }
