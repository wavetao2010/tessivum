...
const FIXTURE = `${import.meta.dir}/snapshots/minimal-preset/session.jsonl`
...
test('sends the exact minimal-preset prompt and schemas through the native agent', async () => {
  const harness = await RustWebHarness.launch({ name: 'minimal-preset-snapshot', replayFixture: FIXTURE })
  try {
    ...
    const created = await harness.rpc<{ sessionId: string }>('session.create', { sessionId: 'minimal-preset-smoke', workspaceId })
