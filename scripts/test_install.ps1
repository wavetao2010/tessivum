...
function Get-VersionVariants {
    ...
    Assert-True -Condition ($SourceVersion.Length -gt 0) -Message 'release version must not be empty'
    $lastCharacter = $SourceVersion[$SourceVersion.Length - 1]
    Assert-True -Condition ($lastCharacter -match '^[1-8]$') -Message "release version must end in a digit from 1 through 8 to construct real-binary upgrade fixtures: $SourceVersion"
    $digit = ([int][char]$lastCharacter) - ([int][char]'0')
    $prefix = $SourceVersion.Substring(0, $SourceVersion.Length - 1)
    $oldVersion = $prefix + ($digit - 1).ToString()
    $newVersion = $prefix + ($digit + 1).ToString()
    return [PSCustomObject]@{
        Old = $oldVersion
        New = $newVersion
    }
