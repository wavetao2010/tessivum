...
import { activeRegion, routesFor, type CatalogSource as CatalogRouteSource, type Region } from './regions.ts'

export type CatalogSource = 'tessivum' | 'dsh-community'
export type TessivumCompatibility = 'official' | 'verified' | 'unverified'
const CATALOG_SOURCES = new Set<CatalogSource>(['tessivum', 'dsh-community'])
const TESSIVUM_COMPATIBILITIES = new Set<TessivumCompatibility>(['official', 'verified', 'unverified'])
...
export interface RegistryPlugin {
  ...
  install: string
  added: string
  /** Whether Tessivum owns this catalog entry or it originated in the DSH community registry. */
...
function sourceKey(source: CatalogRouteSource): string {
...
function asRegistry(value: unknown): Registry {
  ...
  const plugins = data.plugins.map((plugin, index) => {
    ...
    const catalogSource = plugin.catalogSource ?? 'dsh-community'
    const tessivumCompatibility = plugin.tessivumCompatibility ?? 'unverified'
    if (!CATALOG_SOURCES.has(catalogSource) || !TESSIVUM_COMPATIBILITIES.has(tessivumCompatibility)) {
      throw new Error(`catalog plugin ${String(index)} carries invalid Tessivum metadata`)
    }
    return { ...plugin, category, catalogSource, tessivumCompatibility }
  })
  return { ...data, plugins }
}
