...
import { mountMarketRoutes, type HostLifecycle, type MarketConfig, type MarketHost } from './routes.ts'
import type { AgentsServiceLike } from './agents.ts'

export const name = 'tessivum-market'

/** Optional cordis.yml configuration; profile defaults to `web`. */
export type Config = Partial<Pick<MarketConfig, 'profile' | 'maxSnapshots'>>
...
export function apply(ctx: Context, config?: Config): void {
  ctx.inject(['webServer', 'loader'], (hostCtx: Context) => {
    ...
    const hostLifecycle = hostCtx.get('hostLifecycle') as HostLifecycle | undefined
    ...
    if (desktopProfiles === undefined) {
      ...
      host.effect(() => mountMarketRoutes(host, resolved, undefined, agentsLookupOf(ctx)), 'tessivum-market: http routes')
      return
    }
    ...
    hostCtx.inject(['desktopPnpm'], (desktopCtx: Context) => {
      ...
      }, 'tessivum-market: Desktop http routes and package operations')
    })
  })
}
