...
- request kind 闭集：`exit`、`plugin.load`、`plugin.update`、`plugin.dispose`、`plugin.snapshot`、`service.call`、`service.provide`、`service.remove`、`event.subscribe`、`event.emit`、`event.callback`、`registration.dispose`。
- 控制/结果 kind：`hello`、`ready`、`response`、`error`、`cancel`、`heartbeat`、`log`。`response/error/cancel` 回显原 `requestId`；远端错误是 `{ code, message, details? }`，不得降级为无结构字符串。
