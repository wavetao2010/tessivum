...
`v0.1.0-alpha.17` is a prerelease, not a production-stable API or data-format promise. It ships `tessivum-market`, a Tessivum-owned first-party market derived from `dshmarket@1.38.1`, as a checksum-verified release artifact and migrates older `dshmarket` profile entries transactionally.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/wavetao2010/tessivum/v0.1.0-alpha.17/install.sh
sh install.sh 0.1.0-alpha.17
```
...
Download the archive and adjacent `.sha256` file for your platform from the [Alpha.17 release](https://github.com/wavetao2010/tessivum/releases/tag/v0.1.0-alpha.17), verify the checksum, then run either packaged launcher:

```bash
target=x86_64-unknown-linux-gnu  # or aarch64-unknown-linux-gnu, x86_64-apple-darwin, aarch64-apple-darwin
sha256sum -c "tessivum-0.1.0-alpha.17-$target.tar.gz.sha256"
tar -xzf "tessivum-0.1.0-alpha.17-$target.tar.gz"
"./tessivum-0.1.0-alpha.17-$target/bin/tessivum" --version
"./tessivum-0.1.0-alpha.17-$target/bin/tsv" --version
```
